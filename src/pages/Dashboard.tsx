...
const MetricCard = ({ title, value, subtitle, icon }: { title: string, value: string | number, subtitle: string, icon: React.ReactNode }) => {
  return (
...
const SessionsList = ({ sessions }: { sessions: any[] }) => {
  return (
...
const Dashboard: React.FC = () => {
  const { dailyMetrics, watchSessions, loading, error } = useMetrics();
  const [timeRange, setTimeRange] = useState<'week' | 'month'>('week');
  
  // Filter metrics based on time range
  const filteredMetrics = dailyMetrics.slice(0, timeRange === 'week' ? 7 : 30);
  
  // Calculate overall stats
  const totalWatchTime = filteredMetrics.reduce((sum, metric) => sum + metric.total_watch_time, 0);
  const totalFocusTime = filteredMetrics.reduce((sum, metric) => sum + metric.focus_time, 0);
  const totalVideosWatched = filteredMetrics.reduce((sum, metric) => sum + metric.videos_watched, 0);
  const totalDistractions = filteredMetrics.reduce((sum, metric) => sum + metric.distraction_count, 0);
  
  // Calculate average focus percentage
  const avgFocusPercentage = totalWatchTime > 0 
  ...
  const dailyWatchTimeData = filteredMetrics
    .slice()
    .reverse()
    .map(metric => ({
  ...
  const dailyFocusPercentageData = filteredMetrics
    .slice()
    .reverse()
    .map(metric => ({
      label: new Date(metric.date).toLocaleDateString(undefined, { weekday: 'short' }),
    ...
    <div className="container mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mb-6">
        <MetricCard 
          ...
          value={formatTimeDisplay(totalWatchTime)}
          subtitle={`Last ${timeRange === 'week' ? '7' : '30'} days`}
          icon={
        ...
        <MetricCard 
          title="Focus Rate" 
          value={`${avgFocusPercentage}%`}
          subtitle="Time focused vs. total time"
          icon={
            <svg className="h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <MetricCard 
          ...
          subtitle="Number of times distracted"
          icon={
            <svg className="h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        <BarChart 
          ...
          data={dailyWatchTimeData}
        />
        
        <BarChart 
          title="Daily Focus Percentage"
          data={dailyFocusPercentageData}
        />
      </div>
